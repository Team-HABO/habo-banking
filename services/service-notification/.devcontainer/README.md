# Dev Container – Notification Service

The dev container provides a fully configured, reproducible development environment for the **Notification Service**. It runs two containers: the development environment itself (`app`) and a RabbitMQ broker (`rabbitmq`).
...
| `SMTP_*`        | Loaded from `../.env` (relative to `.devcontainer/`, required) |

The `.env` file at `services/service-notification/.env` is required and must exist before the container starts.
Copy `services/service-notification/.env.sample` to `services/service-notification/.env` and fill in the secrets.
...
| `dotnet.defaultSolution`                   | `service-notification.sln` |
